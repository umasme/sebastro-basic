...
def initialize_camera(device_serial=None):
    ...
    # Configure streams - only depth stream for functionality
...
def create_point_cloud(camera, depth_frame=None, visualize=False):
    """
    Create a point cloud from camera frames.
    ...
    # Use uniform coloring for visualization
    # Each point gets a color based on its z-value (depth) for better visualization
    if len(vertices) > 0:
        z_values = vertices[:, 2]  # Extract z component
        if np.max(z_values) - np.min(z_values) > 0:
            ...
            colors[:, 0] = 1 - norm_z  # Red channel (higher for closer points)
            colors[:, 2] = norm_z      # Blue channel (higher for farther points)
        else:
            # Fallback if all points have same z
            colors = np.ones((len(vertices), 3)) * np.array([0.5, 0.5, 0.5])  # Gray
    else:
        colors = np.ones((0, 3)) * np.array([0.5, 0.5, 0.5])  # Empty case
